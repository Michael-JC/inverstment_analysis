...
def test():
    ...
    def test_general_handler(event):
        print(u'处理每秒触发的计时器事件：%s' % str(datetime.now()))
    
    def test_event_handler(event):
        print("event type {}".format(event.type_))
    
    ee = EventEngine()
    ee.start()
    
    test_event = Event('foo')
    ee.registerGeneralHandler(test_general_handler)
    
    ee.register(test_event.type_,test_event_handler)
    while True:
        ee.put(test_event)
        time.sleep(3)
    ee.stop()
