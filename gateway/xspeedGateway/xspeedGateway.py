...
orderStatusMapReverse[defineDict["DFITC_SPD_PARTIAL_CANCELED"]
                      ] = STATUS_CANCELLED
    ...
    def initQuery(self):
        ...
        if self.qryEnabled:
            ...
            self.qryFunctionList = [
                self.qryAccount,
                self.qryFuturesPosition,
                self.qryOptionPosition]
...
class XspeedMdApi(MdApi):
    ...
    def onMarketData(self, data):
        ...
        # '.'.join([tick.symbol, EXCHANGE_UNKNOWN])
...
class XspeedTdApi(TdApi):
    ...
    def onRtnMatchedInfo(self, data):
        ...
        trade.exchange = exchangeMapReverse.get(
            data['exchangeID'], EXCHANGE_UNKNOWN)
        ...
        trade.direction = directionMapReverse.get(
            data['buySellType'], DIRECTION_UNKNOWN)
        trade.offset = offsetMapReverse.get(
            data['openCloseType'], OFFSET_UNKNOWN)
        ...
        order.status = orderStatusMapReverse.get(
    ...
    def onRspQryPosition(self, data, error, last):
        ...
        if data['instrumentID']:
            ...
            positionName = '.'.join(
                [data['instrumentID'], str(data['buySellType'])])
            ...
            if positionName in self.posDict:
                ...
                # 方向
                pos.direction = directionMapReverse.get(
                    data['buySellType'], '')
    ...
    def onRspQryExchangeInstrument(self, data, error, last):
        ...
        contract.exchange = exchangeMapReverse.get(
            data['exchangeID'], EXCHANGE_UNKNOWN)
