...
from ui.uiMainWindow import QtWidgets, QtCore
from .drBase import EVENT_DATARECORDER_LOG
from language.chinese import data_recorder_text
...
class TableCell(QtWidgets.QTableWidgetItem):
    ...
    #----------------------------------------------------------------------
    def __init__(self, data_recorder_text=None, mainEngine=None):
        """Constructor"""
        super(TableCell, self).__init__()
        self.data = None
        self.setTextAlignment(QtCore.Qt.AlignCenter)
        if data_recorder_text:
            self.setContent(data_recorder_text)

    #----------------------------------------------------------------------
    def setContent(self, data_recorder_text):
        """设置内容"""
        if data_recorder_text == '0' or data_recorder_text == '0.0':
            self.setText('')
        else:
            self.setText(data_recorder_text)
...
class DrEngineManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        """初始化界面"""
        self.setWindowTitle(data_recorder_text.DATA_RECORDER)

        # 记录合约配置监控
        tickLabel = QtWidgets.QLabel(data_recorder_text.TICK_RECORD)
        ...
        self.tickTable.setHorizontalHeaderLabels(
            [data_recorder_text.CONTRACT_SYMBOL, data_recorder_text.GATEWAY])

        barLabel = QtWidgets.QLabel(data_recorder_text.BAR_RECORD)
        ...
        activeLabel = QtWidgets.QLabel(data_recorder_text.DOMINANT_CONTRACT)
        ...
        self.activeTable.setHorizontalHeaderLabels(
            [data_recorder_text.DOMINANT_SYMBOL, data_recorder_text.CONTRACT_SYMBOL])
